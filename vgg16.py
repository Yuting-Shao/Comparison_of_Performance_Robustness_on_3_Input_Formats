...
# CS 5330 Spring 2023
# Final Project - Compare jpg/linear/log images for a image recognition using VGG16
...
def main(argv):
    ...
    print(f"Evaluating num of epochs for jpg using VGG16...")
    start_time = time.time()
    test_accuracy = train_and_evaluate(batch_size=16, num_epochs=50, datasets_name=['jpg64DB_trainset', 'jpg64DB_devset', 'jpg64DB_testset'], filename='num_epochs_loss_jpg_vgg16.png', modelname='trained_vgg16_jpg64db')
    end_time = time.time()
    run_time = end_time - start_time
    print(f"Train and evaluate VGG16 took {run_time:.2f} seconds")

    # optimize number of epochs using early stopping
    print(f"Evaluating num of epochs for linear using VGG16...")
    start_time = time.time()
    test_accuracy = train_and_evaluate(batch_size=16, num_epochs=50, datasets_name=['ln64db_trainset', 'ln64db_devset', 'ln64db_testset'], filename='num_epochs_loss_linear_vgg16.png', modelname='trained_vgg16_linear64db')
    end_time = time.time()
    run_time = end_time - start_time
    print(f"Train and evaluate VGG16 took {run_time:.2f} seconds")

    # optimize number of epochs using early stopping
    print(f"Evaluating num of epochs for log using VGG16...")
    start_time = time.time()
    test_accuracy = train_and_evaluate(batch_size=16, num_epochs=50, datasets_name=['log64db_trainset', 'log64db_devset', 'log64db_testset'], filename='num_epochs_loss_log_vgg16.png', modelname='trained_vgg16_log64db')
    end_time = time.time()
    run_time = end_time - start_time
    print(f"Train and evaluate VGG16 took {run_time:.2f} seconds")
