...
# CS 5330 Spring 2023
# Final Project - Check the robustness of the trained ResNet-34: intensity
...
class CustomVariationImageFolder(ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        if self.custom_transforms:
            for transform in self.custom_transforms:
                img_intensity_variation = transform(img_intensity_variation)

                if img_intensity_variation is None:
                    return None, None

        image = torch.from_numpy(np.transpose(img_intensity_variation, (2, 0, 1))).float()
        return image, target
